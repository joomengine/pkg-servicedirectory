# Service Directory Package (v5.0.0)

The Joomla Service Directory Package that bundles JSD Component and all the plugins and features available into one package for easy install and update of your JSD system for Joomla 5.

## Details

- Packager: [Vast Development Method](https://dev.vdm.io/)
- Author: [Llewellyn van der Merwe](https://io.vdm.dev/)
- Creation Date: October 2025
- Max Joomla Version: J5.4
- Minimum Joomla Version: J5.3

## Extensions

- [joomla/Service-Directory](https://git.vdm.robot/joomla/Service-Directory) [5.x](https://git.vdm.robot/joomla/Service-Directory/archive/5.x.zip)
> 5.x
- [joomla/plg_privacy_servicedirectory](https://git.vdm.robot/joomla/plg_privacy_servicedirectory) [5.x](https://git.vdm.robot/joomla/plg_privacy_servicedirectory/archive/5.x.zip)
> 5.x

### Copyright (2015)
> Copyright (C) 2015 Llewellyn van der Merwe. All rights reserved.
### License
> GNU General Public License version 2; see LICENSE

